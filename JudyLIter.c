...
#endif
{
	...
	Pjbu_t Pjbu;
	Pjll_t Pjll = NULL;
	Word_t bitposmask, digit, state, pop1 = 0;
	Pjp_t APjphist[cJL_ROOTSTATE];
	int offset, subexp, Aoffhist[cJL_ROOTSTATE], histnum = 0;
...
#define	SM3PREPB(cState,Next)  state = (cState); goto Next
...
#else
		if ((offset = (~offset)) >= Pjbl->jbl_NumJPs)
#endif
			goto SM2Backtrack;
	...
	case cJL_JPBRANCH_B:
		...
		if (JL_JBB_BITMAP(Pjbb, subexp) & bitposmask) {
			assert(offset >= 0);
...
#define	PJI (Pjp->jp_LIndex)
	...
	case cJL_JPLEAF_B1: {
		...
		assert(subexp < cJL_NUMSUBEXPL);
		if (JL_JLB_BITMAP(Pjlb, subexp) & bitposmask) {
...
#define	SM1IMM_01							  \
	...
	case cJL_JPBRANCH_B:
		...
		goto SM2BranchB;
	      SM2BranchB:
		Pjbb = P_JBB(Pjp->jp_Addr);
		HISTPOPBOFF(subexp, offset, digit);
#ifdef JUDYPREV
		if (offset > 0) {
			--offset;
	...
	case cJL_JPBRANCH_B:
		...
		while (!(JL_JBB_BITMAP(Pjbb, --subexp))) {
			if (subexp <= 0) {
				JL_SET_ERRNO(JLE_CORRUPT);
				return PPJERR;
			}
...
#define	SM3LEAFLNODCD			\
	...
	case cJL_JPLEAF_B1: {
		...
		while (!JL_JLB_BITMAP(Pjlb, --subexp)) {
			if (subexp <= 0) {
				JL_SET_ERRNO(JLE_CORRUPT);
				return PPJERR;
			}
	...
	case cJL_JPIMMED_3_01:
		...
		return (void **)(&Pjp->jp_Addr);
#ifdef JUDYPREV
#define	SM3IMM_OFFSET(cPop1)  (cPop1) - 1
#else
#define	SM3IMM_OFFSET(cPop1)  0
#endif
