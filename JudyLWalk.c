...
#define WALK_FN(_i, _k) do {if ((ret = fn(ctx, _i, _k)) != 0) return ret;} while (0)
#define JUDYWALK(P, _p) do {if ((ret = JudyWalk(P, _p, fn, ctx)) != 0) return ret;} while (0)
	...
	case cJL_JPLEAF_B1: {
		...
		for (i = 0; i < cJL_NUMSUBEXPB; i++) {
			...
			for (mask = 0; mask < cJL_BITSPERSUBEXPB; mask++) {
				...
				Index = (prefix & 0xffffff00) | (i*cJL_BITSPERSUBEXPB+mask);
				WALK_FN(Index, (void **)Pjv2);
			}
		}
		return 0;
	}
...
#define WALK_JPIMMED(pos) Index = prefix | ((uint8_t *)Pjp->jp_LIndex)[pos]; \
WALK_FN(Index, (void **)(P_JV(Pjp->jp_Addr) + pos))
